...
app.get("/filter",(req,res)=>{
  ...
  const alljokes = jokes.filter((joke)=> joke.jokeType === type);
  res.json(alljokes);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if(jokeIndex >-1){
    const deljoke = jokes.splice(jokeIndex, 1);
    console.log(deljoke);
    res.json(deljoke);
  }else{
    res.status(404).json({error : "Joke not found, couldn't delete."});
  }
...
app.delete("/all", (req, res) => {
  ...
  if(key===masterKey){
    ...
    res.status(404).json({error: "Authorization required."});
  }
});
